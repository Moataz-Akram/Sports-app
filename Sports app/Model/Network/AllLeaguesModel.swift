...
//        AF.request(URLs.allLeaguesURL).validate().responseDecodable(of: LeagueResponse.self) { (response) in
...
//        AF.request(url).validate().responseDecodable(of: LeagueDetailResponse.self) { (response) in
//                switch response.result {
//                case .success( _):
//                    guard let leagues = response.value?.leagues else { return }
